...
public class MealFromSpecificAreaAdapter extends RecyclerView.Adapter<MealFromSpecificAreaAdapter.MyViewHolder> {
    List<MealsItem> meals;
    ViewGroup viewGroup;

    public MealFromSpecificAreaAdapter(List<MealsItem> meals) {
        this.meals=meals;
    }
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new MealFromSpecificAreaAdapter.MyViewHolder(view);
    }
                ...
                RetrofitClient.getInstance().getMyApi().getMealById(Integer.parseInt(meals.get(position).getIdMeal()))
                        ...
                        .subscribe(new Observer<RootMeal>() {
                            ...
                            public void onNext(@io.reactivex.rxjava3.annotations.NonNull RootMeal rootMeal) {
                                MealFromSpecificArea.searchTextInput.setText("");
                                Navigation.findNavController(viewGroup).navigate(MealFromSpecificAreaDirections.actionMealByCountryFragmentToMealDeatailsFragment(rootMeal.getMeals().get(0)));
                            }
