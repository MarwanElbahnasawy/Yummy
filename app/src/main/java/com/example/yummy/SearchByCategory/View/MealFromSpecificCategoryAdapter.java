...
public class MealFromSpecificCategoryAdapter extends RecyclerView.Adapter<MealFromSpecificCategoryAdapter.MyViewHolder>{
    List<MealsItem> meals;
    ViewGroup viewGroup;

    public MealFromSpecificCategoryAdapter(List<MealsItem> meals) {
        this.meals=meals;
    }

    @NonNull
    @Override
    public MealFromSpecificCategoryAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
       viewGroup=parent;
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.search_meal_items, parent, false);
        return new MealFromSpecificCategoryAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MealFromSpecificCategoryAdapter.MyViewHolder holder, int position) {
                ...
                RetrofitClient.getInstance().getMyApi().getMealById(Integer.parseInt(meals.get(position).getIdMeal()))
                        ...
                        .subscribe(new Observer<RootMeal>() {
                            ...
                            public void onNext(@io.reactivex.rxjava3.annotations.NonNull RootMeal rootMeal) {
                                MealFromSpecificCategory.searchTextInput.setText("");
                                Navigation.findNavController(viewGroup).navigate(MealFromSpecificCategoryDirections.actionMealByCategoryFragmentToMealDeatailsFragment(rootMeal.getMeals().get(0)));
                            }
